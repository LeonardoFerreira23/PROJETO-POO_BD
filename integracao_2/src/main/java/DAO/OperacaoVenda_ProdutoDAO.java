...
public class OperacaoVenda_ProdutoDAO extends ConnectionDAO{
    ...
    public boolean insertOP_Produto(OperacaoVenda_Produtos produto) {
        ...
        try {
            ...
            pst.setInt(1, produto.getOperacaoVenda_idOperacaoVenda());
            pst.setInt(2, produto.getProdutos_idProduto());
            pst.execute();
            sucesso = true;
    ...
    public boolean deleteProdutos(int id, int id2) {
        connectToDB();
        String sql = "DELETE FROM operacaoVenda_has_produtos where (OperacaoVenda_idOperacaoVenda,Produtos_idProdutos)=(?,?)";
        try {
            pst = con.prepareStatement(sql);
            pst.setInt(1, id);
            pst.setInt(2, id2);
