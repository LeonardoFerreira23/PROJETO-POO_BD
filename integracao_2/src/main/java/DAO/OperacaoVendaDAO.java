...
public class OperacaoVendaDAO extends ConnectionDAO{
    ...
    public boolean insertOperacao(OperacaoVenda operacao) {
        ...
        try {
            ...
            pst.setLong(3, operacao.getCliente_CPF());
            pst.execute();
            sucesso = true;
    ...
    //UPDATE
    public boolean updateOperacao(int id, int id2, long id3) {
        connectToDB();
        String sql = "UPDATE OperacaoVenda SET Vendedores_idVendedores=?, Cliente_CPF=?  where idOperacaoVenda=?";
        try {
            pst = con.prepareStatement(sql);
            pst.setInt(1, id2);
            pst.setLong(2, id3);
    ...
    public boolean deleteOperacao(int id) {
        ...
        try {
            ...
            System.out.println("Erro = Essa operação de venda está sendo referenciada em alguma tabela");
            sucesso = false;
        } finally {
            try {
                con.close();
    ...
    public ArrayList<OperacaoVenda> selectOperacao() {
        ...
        try {
            ...
            while (rs.next()) {
                OperacaoVenda opAux = new OperacaoVenda(rs.getInt("idOperacaoVenda"), rs.getInt("Vendedores_idVendedores"), rs.getLong("Cliente_CPF"));
