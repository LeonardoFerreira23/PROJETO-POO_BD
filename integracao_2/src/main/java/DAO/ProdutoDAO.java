...
public class ProdutoDAO extends ConnectionDAO{
    ...
    //UPDATE
    public boolean updateProdutos(int id, String nome, int valor) {
        connectToDB();
        String sql = "UPDATE Produtos SET Nome = ?, Valor = ? WHERE idProdutos = ?";
        try {
            pst = con.prepareStatement(sql);
    ...
    public ArrayList<Produtos> selectProduto() {
        ...
        try {
            ...
            System.out.println("Erro: O produto está sendo referenciado em alguma tabela" );
            sucesso = false;
        } finally {
            try {
                con.close();
    ...
    public int selectProdutoValor(int id) {
        int valor = 0;
        ArrayList<Produtos> produtos = new ArrayList<>();
        connectToDB();
        String sql = "SELECT * FROM Produtos";
        try {
            ...
            while (rs.next()) {
                Produtos prodAUX = new Produtos(rs.getInt("idProdutos"),rs.getInt("Valor"), rs.getString("Nome"));
                produtos.add(prodAUX);
                if (prodAUX.getIdProdutos() == id)
                    valor = prodAUX.getValor();
            }
            sucesso = true;
        } catch (SQLException e) {
            System.out.println("Erro: O produto está sendo referenciado em alguma tabela" );
