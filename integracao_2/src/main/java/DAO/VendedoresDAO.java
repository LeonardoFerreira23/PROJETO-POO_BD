...
public class VendedoresDAO extends ConnectionDAO{
    ...
    //UPDATE
    public boolean updateVendedores(int id, String nome) {
        connectToDB();
        String sql = "UPDATE Vendedores SET Nome = ? WHERE idVendedores = ?";
        try {
            pst = con.prepareStatement(sql);
    ...
    public boolean deleteVendedores(int id) {
        ...
        try {
            ...
            System.out.println("Erro = O vendedor está sendo referenciado em alguma tabela ");
            sucesso = false;
        } finally {
            try {
                con.close();
