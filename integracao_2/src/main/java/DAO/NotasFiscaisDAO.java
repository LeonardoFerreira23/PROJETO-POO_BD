...
public class NotasFiscaisDAO extends ConnectionDAO{
    ...
    public boolean insertNota(NotasFiscais nota) {
        connectToDB();
        String sql = "INSERT INTO NotasFiscais (idNotasFiscais, OperacaoVenda_idOperacaoVenda, Total) values(?,?,?)";
        try {
            pst = con.prepareStatement(sql);
            pst.setInt(1, nota.getIdNotasFiscais());
            pst.setInt(2, nota.getOperacaoVenda_idOperacaoVenda());
            pst.setInt(3,nota.getTotal());
            pst.execute();
            sucesso = true;
    ...
    public ArrayList<NotasFiscais> selectNotas() {
        ...
        try {
            ...
            while (rs.next()) {
                NotasFiscais notAux = new NotasFiscais(rs.getInt("idNotasFiscais"),rs.getInt("OperacaoVenda_idOperacaoVenda"),rs.getInt("Total"));
                ...
                System.out.println("Total = " + notAux.getTotal());
                System.out.println("--------------------------------");
